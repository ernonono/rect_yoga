...
export default function EditPatient() {
  ...
  const { data, isLoading: loadingPatient } = useQuery({
    queryKey: ["doctor", id],
    queryFn: async () => {
      const { data } = await instance.get(`patients/${id}`);
  ...
  useEffect(() => {
    if (data) {
      ...
      setImageUrl(`http://localhost:8000/patient_image/${data.image}`);
    }
  }, [data]);
  ...
  const mutation = useMutation({
    mutationFn: (body) => instance.put(`patients/${id}`, body),
    onSuccess: () => {
